...
if(isset($_POST['stdemail_data']))
{
 require 'class/class.phpmailer.php';
 $output = '';
 foreach($_POST['stdemail_data'] as $row)
 {
  $mail = new PHPMailer;
  $mail->IsSMTP();        //Sets Mailer to send message using SMTP
  $mail->Host = 'smtp.gmail.com';  //Sets the SMTP hosts of your stdemail hosting, this for Godaddy
  $mail->Port = '587';        //Sets the default SMTP server port
  $mail->SMTPAuth = true;       //Sets SMTP authentication. Utilizes the Username and Password variables
  $mail->Username = 'pratham619buddhadev@gmail.com';     //Sets SMTP username
  $mail->Password = 'Pratham@619';     //Sets SMTP password
  $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;       //Sets connection prefix. Options are "", "ssl" or "tls"
  $mail->From = 'pratham619buddhadev@gmail.com';   //Sets the From stdemail address for the message
  $mail->FromName = 'Pratham';     //Sets the From name of the message
  $mail->AddAddress($row["stdemail"], $row["name"]); //Adds a "To" address
  $mail->WordWrap = 50;       //Sets word wrapping on the body of the message to a given number of characters
  $mail->IsHTML(true);       //Sets message type to HTML
  $mail->Subject = 'Pratham Buddhadev'; //Sets the Subject of the message
  ...
  $result = $mail->Send();      //Send an stdemail. Return true on success or false on error
